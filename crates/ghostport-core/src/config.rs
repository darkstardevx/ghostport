...
/// Which side of a peer pair this config describes.
#[derive(Debug, Deserialize, Serialize, Clone, Copy, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum Role {
    /// Always reachable; accepts both the control connection and every
    /// data-tunnel connection. Never dials the peer itself.
    Server,
    /// Assumed to be behind NAT/unpredictable networks; only ever
    /// dials out, never accepts an inbound connection.
    Client,
}

/// Which direction a link relays traffic in.
#[derive(Debug, Deserialize, Serialize, Clone, Copy, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum LinkMode {
...
pub struct PeerConfig {
    /// Human label, shown in status/logs/the TUI — not part of the
    /// cryptographic identity, just how this peer is referred to.
    pub name: String,
    /// Base64-encoded 32-byte Noise static public key, as printed by
    /// `ghostport keygen` on the peer's own machine.
    pub public_key: String,
    /// Which of this config's `links` (by id) this peer may use.
    pub links: Vec<String>,
}

/// One forwarded port, in one direction ([`LinkMode`]). A link needs a
/// matching entry with the same `id` on both peers' configs.
#[derive(Debug, Deserialize, Serialize, Clone)]
pub struct LinkConfig {
    /// Identifies this link across both sides of the pair and in the
    /// `StreamHello` message that names which link a data tunnel is
    /// for.
    pub id: String,
...
/// The full TOML config for one role of a peer pair — see [`Role`] for
/// which fields apply to which side. Load with [`Config::load`],
/// validate with [`Config::validate`] before trusting it for anything.
#[derive(Debug, Deserialize, Serialize, Clone)]
pub struct Config {
    /// Which side of the pair this config describes.
    pub role: Role,
    /// Path to this identity's own private key file (see the `keys`
    /// module), as generated by `ghostport keygen`.
    pub private_key_path: PathBuf,

    // Client-role field: the one server this client trusts.
    /// Base64-encoded public key of the one server this client trusts.
    /// Client-role only.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Address the control channel listens on. Server-role only.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Config {
    /// Reads and parses a config file. Does *not* validate it — call
    /// [`Config::validate`] separately before trusting the result.
