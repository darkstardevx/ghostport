        ...
        Role::Server => {
            for peer in &cfg.peers {
                if let Ok(peer_key) = keys::decode_public_key(&peer.public_key) {
                    println!("  peer \"{}\" fingerprint: {} (links: {})", peer.name, theme::accent(&keys::fingerprint(&peer_key)), peer.links.join(", "));
                }
            }
        }
    }
    ...
    let result = match role {
        Role::Server => {
            ...
            for peer in &cfg.peers {
                ...
                peers.push(peermatch::ResolvedPeer { name: peer.name.clone(), public_key, links: peer.links.iter().cloned().collect() });
            }
            let config = Arc::new(cfg);
            server::run(server::Context { config, private_key, peers: Arc::new(peers), state, socket_path }).await
        }
        Role::Client => {
            let peer_public_key = match keys::decode_public_key(cfg.peer_public_key.as_deref().expect("validated: client role requires peer_public_key")) {
                Ok(k) => k,
                Err(e) => {
            ...
            let config = Arc::new(cfg);
            client::run(client::Context { config, private_key, peer_public_key: Arc::new(peer_public_key), state, socket_path }).await
        }
    };
...
mod integration_tests {
    ...
    async fn forward_and_reverse_links_round_trip_end_to_end() {
        ...
        let server_config = Config {
            ...
            peers: vec![config::PeerConfig { name: "client".to_string(), public_key: client_pub_b64, links: vec!["fwd".to_string(), "rev".to_string()] }],
        ...
        tokio::spawn(server::run(server::Context {
            ...
            peers: Arc::new(vec![peermatch::ResolvedPeer { name: "client".to_string(), public_key: client_kp.public.clone(), links: ["fwd", "rev"].map(String::from).into() }]),
    ...
    async fn wrong_peer_key_fails_the_handshake_instead_of_connecting() {
        ...
        let server_config = Config {
            ...
            peer_public_key: None,
            peers: vec![
                config::PeerConfig { name: "client".to_string(), public_key: keys::encode_public_key(&client_kp.public), links: vec![] },
                config::PeerConfig { name: "decoy".to_string(), public_key: keys::encode_public_key(&decoy_kp.public), links: vec![] },
            ],
            listen_control: Some(format!("127.0.0.1:{control_port}")),
            listen_data: Some(format!("127.0.0.1:{data_port}")),
            server_control_addr: None,
        ...
        tokio::spawn(server::run(server::Context {
            ...
            private_key: Arc::new(server_kp.private),
            peers: Arc::new(vec![
                peermatch::ResolvedPeer { name: "client".to_string(), public_key: client_kp.public.clone(), links: Default::default() },
                peermatch::ResolvedPeer { name: "decoy".to_string(), public_key: decoy_kp.public.clone(), links: Default::default() },
            ]),
            state,
        ...
        assert!(result.is_err(), "handshake must fail for a key not on the allowed list");
        ...
        let result = snowstorm::NoiseStream::handshake(tcp, handshake).await;
        assert!(result.is_err(), "handshake must fail against the wrong pinned server key");
    }
    ...
    async fn control_listener_rejects_connections_beyond_the_per_ip_rate_limit() {
        ...
        let server_config = Config {
            ...
            private_key_path: std::path::PathBuf::new(),
            peer_public_key: None,
            peers: vec![config::PeerConfig { name: "client".to_string(), public_key: keys::encode_public_key(&client_kp.public), links: vec![] }],
            listen_control: Some(format!("127.0.0.1:{control_port}")),
            listen_data: Some(format!("127.0.0.1:{data_port}")),
            server_control_addr: None,
            server_data_addr: None,
        ...
        tokio::spawn(server::run(server::Context {
            config: Arc::new(server_config),
            private_key: Arc::new(server_kp.private),
            peers: Arc::new(vec![peermatch::ResolvedPeer { name: "client".to_string(), public_key: client_kp.public.clone(), links: Default::default() }]),
            state,
    ...
    async fn peer_is_rejected_from_a_link_outside_its_own_allowlist() {
        ...
        let server_config = Config {
            ...
            peer_public_key: None,
            peers: vec![
                config::PeerConfig { name: "peer-a".to_string(), public_key: keys::encode_public_key(&peer_a_kp.public), links: vec!["link-a".to_string()] },
                config::PeerConfig { name: "peer-b".to_string(), public_key: keys::encode_public_key(&peer_b_kp.public), links: vec!["link-b".to_string()] },
            ],
            listen_control: Some(format!("127.0.0.1:{control_port}")),
            listen_data: Some(format!("127.0.0.1:{data_port}")),
            server_control_addr: None,
            server_data_addr: None,
            links: vec![
                LinkConfig { id: "link-a".to_string(), mode: config::LinkMode::Forward, listen: None, target: Some(format!("127.0.0.1:{link_a_target_port}")) },
                LinkConfig { id: "link-b".to_string(), mode: config::LinkMode::Forward, listen: None, target: Some(format!("127.0.0.1:{link_b_target_port}")) },
            ],
        };
        assert!(server_config.validate().is_empty(), "{:?}", server_config.validate());
        let server_state = Arc::new(stats::SharedState::new(&server_config));
        ...
        tokio::spawn(server::run(server::Context {
            ...
            peers: Arc::new(vec![
                peermatch::ResolvedPeer { name: "peer-a".to_string(), public_key: peer_a_kp.public.clone(), links: ["link-a".to_string()].into() },
                peermatch::ResolvedPeer { name: "peer-b".to_string(), public_key: peer_b_kp.public.clone(), links: ["link-b".to_string()].into() },
            ]),
            state: server_state.clone(),
        ...
        let client_a_config = Config {
            ...
            server_control_addr: Some(format!("127.0.0.1:{control_port}")),
            server_data_addr: Some(format!("127.0.0.1:{data_port}")),
            links: vec![LinkConfig { id: "link-a".to_string(), mode: config::LinkMode::Forward, listen: Some(format!("127.0.0.1:{client_a_local_port}")), target: None }],
        };
        assert!(client_a_config.validate().is_empty(), "{:?}", client_a_config.validate());
        ...
        let echoed = round_trip(&format!("127.0.0.1:{client_a_local_port}"), b"hello-from-peer-a").await;
        assert_eq!(echoed, b"hello-from-peer-a");
        assert_eq!(server_state.links["link-a"].total_streams.load(std::sync::atomic::Ordering::Relaxed), 1);
        ...
        let mut tunnel = snowstorm::NoiseStream::handshake(tcp, handshake).await.expect("peer A is a real allowed peer -- the handshake itself must still succeed");
        framing::send_json(&mut tunnel, &protocol::StreamHello { link_id: "link-b".to_string(), stream_id: None }).await.unwrap();
        ...
        let mut buf = Vec::new();
        let read_result = tokio::time::timeout(Duration::from_secs(3), tunnel.read_to_end(&mut buf)).await;
        assert!(read_result.is_ok(), "server must close an unauthorized link request, not leave it hanging open");
        assert!(buf.is_empty(), "peer A must not get a working relay for link-b, which it's not authorized for");
        assert_eq!(server_state.links["link-b"].total_streams.load(std::sync::atomic::Ordering::Relaxed), 0, "an unauthorized attempt must not count as a real stream");
    }
}
