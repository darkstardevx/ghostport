...
/// Reads the first handshake message directly off `tcp` and tries it
/// against each of `peers` in turn. Returns the matched peer's index
/// and the now-advanced `HandshakeState` (message 1 already
/// processed, ready to write message 2) — hand it straight to
/// `NoiseStream::handshake` to finish the exchange.
